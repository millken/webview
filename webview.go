...
func (handle *webview) Run() {
	C.prtn_event_loop()
}

func (handle *webview) SetTitle(title string) {
...
func (handle *webview) SetMenuBarExtraText(text string) {
...
func (handle *webview) AddMenuBarExtra(text string, tag int) {
	c_text := C.CString(text)
	defer C.free(unsafe.Pointer(c_text))
...
func (handle *webview) SetContent(content string) {
...
func (handle *webview) SetContentPath(path string) {
...
func (handle *webview) BindWithReply(name string, callbackwithreply CallbackFunc) {
...
func (hanle *webview) ExecuteScript(script string) {
